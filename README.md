# pigpen-masonic-scytale-ciphers

#1.How to Run the Program

#Make sure Python is installed
python --version
#Clone the repo
git clone https://github.com/sarakycyku/pigpen-masonic-scytale-ciphers.git
cd pigpen-masonic-scytale-ciphers
#Run the Program
python -m main.main
#When you run the program, you will see the following menu where you have to choose the algorithm you want to encrypt or decrypt:
========================================
CLASSICAL ENCRYPTION ALGORITHMS
========================================
1. Pigpen
2. Scytale
3. Masonic
4. Exit
========================================
After selecting an algorithm (1-3), you will see:
1. Encrypt
2. Decrypt
Choose:
where encrypt	converts your plaintext message into ciphertext using the selected algorithm and decrypt	converts ciphertext back into plaintext using the selected algorithm.If you choose option 4 the program ends and if u choose 5 or higher it shows an error.
 
